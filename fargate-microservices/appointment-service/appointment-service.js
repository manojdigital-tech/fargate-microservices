...
app.get('/appointments/:id', (req, res) => {
  ...
  if (appointment) res.json({ message: 'Appointment found', appointment });
...
app.post('/appointments', (req, res) => {
  try {
    ...
    if (!patientId || !date || !time || !doctor) return res.status(400).json({ error: 'Patient ID, date, time, and doctor are required' });
...
app.get('/appointments/patient/:patientId', (req, res) => {
  try {
    ...
    if (patientAppointments.length > 0) {
      res.json({ message: `Found ${patientAppointments.length} appointment(s) for patient ${patientId}`,
